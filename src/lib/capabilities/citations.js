// Citation / reference registry (client mirror).
// MIRROR of server/capabilities/citations.js — keep the data identical when editing.
//
// strength: 'VERIFIED' (source-checked), 'MIXED' (real source but the effect is
// replication-contested or only correlational — surfaced WITH the caveat), 'KNOWN'
// (canonical-from-training, NOT yet source-checked — carries needsReverify and must
// not render as established fact). Day 6 re-verified every previously-KNOWN entry.
...
export function citationsForDomain(domainKey) {
